...
KL散度（又称相对熵）用来**衡量两个概率分布 $P$ 和 $Q$ 之间的差异**。
...
- **KL散度不是对称的**，即 $D_{KL}(P\|Q) \neq D_{KL}(Q\|P)$，**所以它不是严格意义上的“距离”**。这意味着 KL 散度在不同的方向上关注的重点不同：
  - $D_{KL}(P\|Q)$强调覆盖真实分布$P$的高概率区域
  - $D_{KL}(Q\|P)$强调避免在$P$低概率区域生成内容
...
以下从不同视角说明来帮助理解 KL 散度。

**信息论视角**

- 信息量：一个事件$x$的信息量可以表示为$I(x)=-\log P(x)$
- KL 散度衡量如果我们用$Q$来近似$P$，平均每次事件会额外浪费多少信息量

$$
D_{KL}(P\|Q)=\mathbb{E}_{x\sim P}[\log P(x) - \log Q(x)]
$$

- 直观理解：如果$Q$完全等于$P$，KL 散度为 0（没有额外信息损失）
- 如果$Q$和$P$差异大，KL 散度就大，说明用$Q$来预测$P$会损失很多信息

**期望视角**

- KL 散度可以看作$P$下的对数似然比的期望：

$$
D_{KL}(P\|Q)=\mathbb{E}_{x\sim P}[\log \frac{P(x)}{Q(x)}]
$$

- 概率论上，它衡量了“用$Q$来代替$P$的期望损失”。
