...
> 前馈层很简单。标准 Transformer 的前馈层通常是一个`两层全连接 + 激活函数`：
> 
> $$
...
> 
> 其中：
> - 输入 $x\in \mathbb{R}^{d_{model}}$
> - $W_1\in \mathbb{R}^{d_{model}\times d_{ff}}, W_2\in \mathbb{R}^{d_{ff}\times d_{model}}$
> - $d_{ff}$ 通常大于 $d_{model}$，比如 $4\times$，增加表达能力
> - $\sigma$ 是激活函数，一般用 ReLU 或者 GeLU

**为什么需要前馈层**

> - 非线性变换：注意力层只是线性加权，FFN 提供非线性表达能力
> - 跨维度混合：FFN 在每个 token 的 embedding 内部做“特征组合”，学习复杂的 token 内部模式
> - 增加模型容量：通过 $d_{ff}>d_{model}$ 增大中间隐藏维度
> - 可以理解为：**注意力学习 token 间关系，前馈层学习 token 内部特征。**

### MoE (Mixture of Experts)

MoE 是一种动态路由机制，它把前馈层扩展成多专家网络：
- 假设有 E 个专家，每个专家是一个独立的 FFN
- 对于每个 token，由一个 **gating network** 选择 1~2 个专家来处理
- 不同 token 可以走不同专家，每个 token 只计算部分专家 -> 提升容量同时控制计算量

数学表达：

$$
MoEFFN(x)=\sum_{i=1}^E G_i(x) \cdot FFN_i(x)
$$

其中：
- $G_i(x)$ 是 gating network 输出的概率（通常 top-k）
- 只选择 top-k 的专家计算，其他专家不计算 -> 节省 FLOPs
...
class SimpleMoE(nn.Module):
    """
    简单 MoE 层，基于 FFN + Top-1 gating
    """
    def __init__(self, d_model, d_ff, num_experts=4, use_aux_loss=True):
        super().__init__()
        ...
        # gating network，输出每个 token 的专家概率
    ...
    def forward(self, x, return_aux=False):
        ...
        # 1. 计算 gating 概率
        gate_logits = self.gate(x)                  # [B, L, num_experts]   # [2, 10, 4]
        gate_probs = F.softmax(gate_logits, dim=-1) # [B, L, num_experts]   # [2, 10, 4] 每个 token 分配给每个专家的概率，和为1

        # 2. Top-1 routing
        top1_val, top1_idx = gate_probs.max(dim=-1) # [B, L] token选择的专家索引

        # 3. 为每个 token 调用对应专家
        out = torch.zeros_like(x)  # [B, L, d_model]  # [2, 10, 16]
        for i in range(self.num_experts):
            # 找到分配给第 i 个专家的 token
            mask = (top1_idx == i)                  # [B, L] bool
            if mask.any():
                selected_x = x[mask]               # [num_tokens, d_model]
                expert_out = self.experts[i](selected_x)
                # 让 gate 有梯度：用 top1 概率对专家输出加权
                out[mask] = expert_out * top1_val[mask].unsqueeze(-1)

        # 4. 可选负载均衡辅助损失（简化版），让路由更均匀，减少专家塌缩
        aux_loss = None
        if self.use_aux_loss or return_aux:
            # token 分配比例
            me = F.one_hot(top1_idx, num_classes=self.num_experts).float()  # [B, L, E]
            me = me.mean(dim=(0, 1))  # [E]
            # gate 概率平均
            ce = gate_probs.mean(dim=(0, 1))  # [E]
...
if __name__ == "__main__":
    ...
    moe = SimpleMoE(d_model, d_ff, num_experts)
